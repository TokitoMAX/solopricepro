 ...
 * v1.8 - FINAL REPAIR - CLEAN & ROBUST
 * Includes visual markers and console logs.
 */
console.log('📡 [MARKETPLACE-v1.8] Script loading...');

const Marketplace = {
    render(containerId = 'marketplace-content') {
        console.log('📡 [MARKETPLACE-v1.8] Render called');
        ...
        container.innerHTML = `
            <div class="page-header" style="margin-bottom: 2rem;">
                <div>
                    <h1 class="page-title" style="display: flex; align-items: center; gap: 10px;">
                        MARKETPLACE PRO <span class="badge-pro" style="background: var(--primary); font-size: 0.7rem; color: white; padding: 4px 8px; border-radius: 6px;">DOMTOM CONNECT</span>
                    </h1>
                    <p class="page-subtitle">Opportunités et collaborations v1.8.</p>
                </div>
            ...
            <div id="marketplace-dynamic-content" class="marketplace-container">
                <p style="text-align: center; color: var(--text-muted); padding: 4rem;">Initialisation du Radar...</p>
            </div>
        `;

        setTimeout(() => this.renderMissions(), 100);
    },
    ...
    renderMissions() {
        console.log('📡 [MARKETPLACE-v1.8] Rendering list');
        ...
        if (missions.length === 0) {
            container.innerHTML = `
                <div class="empty-state" style="text-align: center; padding: 4rem 1rem; background: rgba(255,255,255,0.01); border-radius: 20px; border: 1px dashed var(--border);">
                    ...
                    <p style="color: var(--text-muted); font-size: 0.9rem;">Revenez plus tard pour voir les nouvelles opportunités.</p>
                </div>
            `;
            return;
        }
        ...
        container.innerHTML = `
            ...
            return `
                        <div class="mission-card elite-card" style="padding: 1.5rem; border-radius: 20px; background: rgba(255,255,255,0.02); border: 1px solid ${isOwner ? 'var(--primary-glass)' : 'rgba(255,255,255,0.05)'};">
                            ...
                            <div style="font-size: 1.5rem; color: var(--primary); font-weight: 800; margin-bottom: 1rem;">${budget}€</div>
                            <div style="display: flex; gap: 0.5rem;">
                                ${isOwner ? `
                                    <button class="button-secondary" style="flex: 1;" onclick="Marketplace.editMission('${m.id}')">Modifier</button>
                                ` : `
                                    <button class="button-primary" style="flex: 1;" onclick="Marketplace.applyForMission('${m.id}')">Proposer</button>
                                `}
                            </div>
                        </div>
                    `;
            ...
            <style>
                .elite-card { transition: transform 0.2s ease; }
                .elite-card:hover { transform: translateY(-5px); }
            </style>
        `;
    },
    ...
    showPostMissionForm(missionData = null) {
        ...
        container.innerHTML = `
            <div class="glass" style="padding: 2rem; border-radius: 20px; margin-bottom: 2rem; border: 1px solid var(--border);">
                <h3>${missionData ? 'Modifier' : 'Nouvelle Mission'}</h3>
...
console.log('📡 [MARKETPLACE-v1.8] Initialized.');
