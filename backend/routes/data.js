...
router.post('/:table', async (req, res) => {
    ...
    const isSingularTable = ['settings', 'calculator_data', 'journal', 'sp_settings', 'sp_calculator_data', 'sp_journal', ...profileVariations].includes(table);
...
router.post('/storage/upload/logos', upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        if (uploadError) {
            ...
            return res.status(400).json({
                success: false,
                message: "L'envoi vers Supabase a échoué. Vérifiez le type et la taille de l'image (max 10Mo).",
    ...
    try {
        ...
        if (!buckets.find(b => b.id === 'avatars')) {
            ...
            const { error: createError } = await supabase.storage.createBucket('avatars', {
                ...
                fileSizeLimit: 10485760 // 10MB for avatars
            });
            if (createError) throw createError;
        }
        ...
        if (uploadError) {
            ...
            return res.status(400).json({
                success: false,
                message: "L'envoi de l'avatar a échoué. Max 10Mo.",
                error: uploadError.message
            });
        }
